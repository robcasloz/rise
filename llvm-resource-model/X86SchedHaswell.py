...
def main():
    ...
    schedHaswellTD = open('input/X86SchedHaswell.td').read()

    #Find all WriteRes defs
    writeResDefs = getWriteResDefs(writeResDefParser, schedHaswellTD)
    #Find all verbose WriteRes defs
    writeResVerboseDefs = getWriteResVerboseDefs(writeResVerboseDefParser, schedHaswellTD)
    #Find all SKLWriteResPair defs
    sklWriteResPairDefs = getSklWriteResPairDefs(sklWriteResPairDefParser, schedHaswellTD)
    #Find all SKLWriteResGroup defs
    sklWriteResGroupDefs = getSklWriteResGroupDefs(sklWriteResGroupDefParser, schedHaswellTD)
    #Find all instructions defined for skylake by llvm
    llvmInstructions = getLlvmInstructions(llvmInstructionParser, schedHaswellTD)
...
def getSklWriteResPairDefParser():
    return pp.Suppress("defm : HWWriteResPair<") + pp.SkipTo(",")("Name") + pp.Suppress(",") +  pp.SkipTo(",")("Resources") + pp.Suppress(",") + pp.SkipTo(">")("Latency")
...
#Find all WriteRes defs
def getWriteResDefs(writeResDef, schedHaswellTD):
    writeResDefs = []
    for writeRes in writeResDef.searchString(schedHaswellTD):
...
#Find all verbose WriteRes defs
def getWriteResVerboseDefs(writeResVerboseDef, schedHaswellTD):
    writeResVerboseDefs = []
    for writeRes in writeResVerboseDef.searchString(schedHaswellTD):
        #Pretty up the parsed data
...
#Find all SKLWriteResPair defs
def getSklWriteResPairDefs(sklWriteResPairDef, schedHaswellTD):
    sklWriteResPairs = []
    for sklWriteResPair in sklWriteResPairDef.searchString(schedHaswellTD):
        tempDict = {
...
#Find all SKLWriteResGroup defs
def getSklWriteResGroupDefs(sklWriteResGroupDef, schedHaswellTD):
    sklWriteResGroupDefs = []
    for sklWriteResGroup in sklWriteResGroupDef.searchString(schedHaswellTD):
        tempDict = {
...
#Find all instructions defined for skylake by llvm
def getLlvmInstructions(instrGroupDef, schedHaswellTD):
    instructions = [] 
    for instrGroup in instrGroupDef.searchString(schedHaswellTD):
